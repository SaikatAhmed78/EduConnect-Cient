...
import { FaHome, FaPlus, FaEdit, FaCalendarAlt, FaUsers, FaUserShield, FaBars } from 'react-icons/fa';
...
const Dashboard = () => {
    ...
    useEffect(() => {
        if (isAdmin) {
            navigate("/dashboard/adminDa");
        } else if (isTutor) {
            navigate("/dashboard/tutorDS");
        } else {
            navigate("/dashboard/studentDS");
        }
  ...
  <NavLink to="/" className={({ isActive }) =>
    `flex items-center space-x-3 p-2 rounded-md transition-all duration-300 ${isActive ? "bg-white text-indigo-600 shadow" : "text-white hover:bg-indigo-700 hover:bg-opacity-50"}`}>
